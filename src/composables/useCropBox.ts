...
export function useCropBox() {
  ...
  const cropBoxElementMobile = ref<HTMLElement | null>(null)
  const cropBoxElementDesktop = ref<HTMLElement | null>(null)
  
  // 获取当前活动的裁剪框元素
  const getCurrentCropBoxElement = (): HTMLElement | null => {
    // 检测屏幕尺寸，lg断点是1024px
    const isDesktop = window.innerWidth >= 1024
    return isDesktop ? cropBoxElementDesktop.value : cropBoxElementMobile.value
  }
  ...
  const startDrag = (event: MouseEvent | TouchEvent) => {
    ...
    // 添加拖拽样式
    const currentCropBox = getCurrentCropBoxElement()
    if (currentCropBox) {
      currentCropBox.classList.add('dragging')
    }
  ...
  const stopDrag = () => {
    ...
    const currentCropBox = getCurrentCropBoxElement()
    if (currentCropBox) {
      currentCropBox.classList.remove('dragging')
    }
  ...
  const startResize = (event: MouseEvent | TouchEvent, handle: string) => {
    ...
    const currentCropBox = getCurrentCropBoxElement()
    if (currentCropBox) {
      currentCropBox.classList.add('resizing')
    }
  ...
  const stopResize = () => {
    ...
    const currentCropBox = getCurrentCropBoxElement()
    if (currentCropBox) {
      currentCropBox.classList.remove('resizing')
    }
  ...
  return {
    ...
    isActive,
    cropBoxStyle,
    cropBoxElementMobile,
    cropBoxElementDesktop,
    getCurrentCropBoxElement,
    startDrag,
    startResize,
    resetCropBox,
