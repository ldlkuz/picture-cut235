...
export function useCropBoxMobile() {
  ...
  const getContainerSize = (): number => {
    const previewContainer = document.querySelector('.preview-container-mobile') as HTMLElement
  ...
  const startDrag = (event: MouseEvent | TouchEvent) => {
    ...
    // 获取预览容器的边界矩形
    const previewContainer = document.querySelector('.preview-container-mobile')
    if (previewContainer) {
      const rect = previewContainer.getBoundingClientRect()
      dragStartPos.value = {
